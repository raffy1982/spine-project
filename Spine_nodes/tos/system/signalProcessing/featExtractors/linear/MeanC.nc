...
generic module MeanC(uint16_t elemCount) {
       uses interface Linear as Lin;

       provides interface Mean;
}
...
implementation {
       ...
       */
       command int32_t Mean.calculate(uint16_t* data) {
            return call Lin.calculate(data, elemCount, coeff, multiplierNum, multiplierDen, shift);
       }

       command error_t Mean.init() {
            for(i=0; i<elemCount; i++)
                 coeff[i] = 1;
            multiplierNum = 1;
            multiplierDen = elemCount;
            shift = 0;
