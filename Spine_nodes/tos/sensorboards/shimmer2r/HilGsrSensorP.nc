...
implementation{
	...
	event void Boot.booted() {
                call SensorsRegistry.registerSensor(GSR_SENSOR);
		call shimmerAnalogSetup.addGSRInput();
		//call GsrInit.init();
		call Gsr.setRange(currRange);
	...
	command error_t Sensor.acquireData(enum AcquireTypes acquireType) {
		...
		if(firstTime){
			call shimmerAnalogSetup.finishADCSetup(gsrData);
			firstTime = FALSE;
		}
	...
	command uint16_t Sensor.getSensorID() {
		return 0x2442; // the ID has been randomly choosen
	}
