...
implementation {
  #define DEFAULT_DEST		SPINE_BASE_STATION;
  uint8_t outSequenceNr = 0;

  command void PacketManager.build(enum PacketTypes pktType, void* pld, uint8_t len) {
    uint8_t fragmentNr = 1;
    uint8_t builtLen = 1;
    uint8_t* payload = call OutPackets.build[pktType](pld, len, &builtLen);
    uint8_t totFragments = ((builtLen % SPINE_PKT_PAYLOAD_MAX_SIZE) == 0) ? (builtLen/SPINE_PKT_PAYLOAD_MAX_SIZE) :
                                                                           ((builtLen/SPINE_PKT_PAYLOAD_MAX_SIZE) + 1);
    spine_header_t* header = call Header.build(SPINE_VERSION, FALSE, pktType, 
                                               GROUP_ID, TOS_NODE_ID, DEFAULT_DEST,
                                               outSequenceNr, fragmentNr, totFragments);

    for(fragmentNr; i<=totFragments; fragmentNr++) {
    ...
    outSequenceNr++;  // seqNr is incremented on a per message basis (not each fragment)
  }
