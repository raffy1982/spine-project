...
public class SpineEMUMessage extends net.tinyos.message.Message {
	...
	protected SpineEMUMessage(byte pktType, byte groupID, int sourceID, int destID, byte sequenceNumber, byte fragmentNr, byte totalFragments, byte[] payload) {
	 ...
	 */
	public static SpineEMUMessage Construct(byte[] rawsfmessage) {
		byte[] payload;
		...
		return new SpineEMUMessage(h.getPktType(), h.getGroupID(), h.getSourceID(), h.getDestID(), h.getSequenceNumber(), h.getFragmentNumber(), h.getTotalFragments(), payload);
	}
	...
	protected EMUMessage parse() throws IllegalSpineHeaderSizeException {
