...
public class DisableFunctionPanel extends javax.swing.JPanel implements ActionListener {
	 ...
	 * 
	 */
	public DisableFunctionPanel(SpineGUI sg, Node n) {
		super();
		this.node = n;
		this.sg = sg;
		initGUI();
	...
	private void initGUI() {
		try {
			...
			this.setPreferredSize(new java.awt.Dimension(587, 529));
			{
				...
				URL imageURL = SetupFunctionPanel.class.getResource("/img/DisableFunction.gif");
			...
			}
			{
				...
				}
				{
					...
					int vv[] = new int[] { 15, 6, 10, 2, 12, 4, 8, 7 };
					for (int i = 0; i < 8; i++)
						ll.addFirst(encodeDeactivation(vv[i]));
		...
		if (e.getSource().equals(jButtonConfirm)) {
			...
			try {
				...
				case SPINEFunctionConstants.FEATURE: {
					...
					fsfr.setActivationFlag(false);
					fsfr.setSensor(sensor);
					if (MaxCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.MAX, (byte) decodeDeactivation(maxComboBox.getSelectedItem().toString())));
					}
					if (minCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.MIN, (byte) decodeDeactivation(minComboBox.getSelectedItem().toString())));
					}
					if (RawDataCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.RAW_DATA, (byte) decodeDeactivation(rawDataComboBox.getSelectedItem().toString())));
					}
					if (RangeCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.RANGE, (byte) decodeDeactivation(rangeComboBox.getSelectedItem().toString())));
					}
					if (MeanCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.MEAN, (byte) decodeDeactivation(meanComboBox.getSelectedItem().toString())));
					}
					if (AmplitudeCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.AMPLITUDE, (byte) decodeDeactivation(amplitudeComboBox.getSelectedItem().toString())));
					}
					if (RmsCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.RMS, (byte) decodeDeactivation(rmsComboBox.getSelectedItem().toString())));
					}
					if (varianceCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.VARIANCE, (byte) decodeDeactivation(varianceComboBox.getSelectedItem().toString())));
					}
					if (ModeCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.MODE, (byte) decodeDeactivation(modeComboBox.getSelectedItem().toString())));
					}
					if (MedianCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.MEDIAN, (byte) decodeDeactivation(medianaComboBox.getSelectedItem().toString())));
					}
					if (pitchCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.PITCH_ROLL, (byte) decodeDeactivation(pitchComboBox.getSelectedItem().toString())));
					}
					if (vectorMagnitudeCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.VECTOR_MAGNITUDE, (byte) decodeDeactivation(vectMagniComboBox.getSelectedItem().toString())));
					}
					if (stdDevCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.ST_DEV, (byte) decodeDeactivation(stdDevComboBox.getSelectedItem().toString())));
					}
					if (TotEnergyCheckBox.isSelected()) {
						fsfr.remove(new Feature(SPINEFunctionConstants.TOTAL_ENERGY, (byte) decodeDeactivation(totEneComboBox.getSelectedItem().toString())));
					}

					int res = JOptionPane.showConfirmDialog(this, "Are you sure about the parameters? \n" + fsfr, "Confirm", JOptionPane.YES_NO_OPTION);
					if (res == JOptionPane.OK_OPTION) {
						sg.deactivateFunction(node, fsfr);
						sg.closeFrame();
						return;
					}
					break;
	...
	private String encodeDeactivation(int i) {
		String s = "";
		switch (i) {
		case 15:
			s = "deactive all";
	...
	private int decodeDeactivation(String g) {
		if (g.equals("deactive all"))
			return 15;
		if (g.equals("deactive Z Y"))
			return 6;
		if (g.equals("deactive Z X"))
			return 10;
		if (g.equals("deactive Z"))
			return 2;
		if (g.equals("deactive Y X"))
			return 12;
		if (g.equals("deactive Y"))
			return 4;
		if (g.equals("deactive X"))
			return 8;
		if (g.equals("deactive null"))
			return 0;
		return 0;
